<?php include('inc/header.php'); ?>
    ...
    <table id="example1" class="table table-bordered table-striped">
                    ...
                    $_top_event_id=(isset($_SESSION['selected_event_id']))?$_SESSION['selected_event_id']:0;
                        $categories=get_all_categories("eventid=$_top_event_id");
                        for($i=0;$i<sizeof($categories);$i++){
                    ?>
                        <tr>
                            <td><?=$categories[$i]['catname']?></td>
                            <td><?=$categories[$i]['parent_cat']?></td>
                            
                            <td><?=$categories[$i]['statusname']?></td>
                            <td><?=action_buttons('addcategory.php','addcategory.php','addcategory.php',$categories[$i]['catid'])?></td>
                        </tr>                      
                    <?php
                        }
                    ?>
