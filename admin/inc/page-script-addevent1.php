...
<script type="text/javascript" src="alpaca/form-builder.js"></script>
    <script>
      ...
      function addSponsorImage(obj){
        ...
        if(noOfSponsors>$('#divSponsorImage').find('input[type=file]').length){
          noOfSponsors=noOfSponsors-$('#divSponsorImage').find('input[type=file]').length;
          for(i=0;i<noOfSponsors;i++){
            $('#divSponsorImage').append(textbox);
          }     
          $('#divSponsorImage').find('input[type=file]').focus(function(){
            $(this).css('border','');
            $(this).css('background','');
          });   
        }
        else if(noOfSponsors<$('#divSponsorImage').find('input[type=file]').length){
          noOfSponsors=$('#divSponsorImage').find('input[type=file]').length-noOfSponsors;        
          for(i=0;i<noOfSponsors;i++){
            _text=$('#divSponsorImage').find('input').last().val();
            if(_text.trim().length<=0){
              $('#divSponsorImage').find('input[type=file]').last().remove();
            }else{              
              $('#divSponsorImage').find('input[type=file]').last().css('border','1px solid #ff0000');
              $('#divSponsorImage').find('input[type=file]').last().css('background','#ffebf6');
              $(obj).val(0);
      ...
      function addHallURL(obj){
        ...
        if(noOfHalls>$('#divHallURL').find('input[type=text]').length){
          noOfHalls=noOfHalls-$('#divHallURL').find('input[type=text]').length;
          for(i=0;i<noOfHalls;i++){
            _textbox=textbox.replace("{0}",$('#divHallURL').find('input[type=text]').length+1);
        ...
        else if(noOfHalls<$('#divHallURL').find('input[type=text]').length){
          noOfHalls=$('#divHallURL').find('input[type=text]').length-noOfHalls;        
          for(i=0;i<noOfHalls;i++){
            _text=$('#divHallURL').find('input[type=text]').last().val();
            if(_text.trim().length<=0){
              $('#divHallURL').find('strong').last().remove();
              $('#divHallURL').find('input[type=text]').last().remove();
            }else{              
              $('#divHallURL').find('input[type=text]').last().css('border','1px solid #ff0000');
              $('#divHallURL').find('input[type=text]').last().css('background','#ffebf6');
              $(obj).val(0);
              alert('Field is not empty');
              return;
            }
